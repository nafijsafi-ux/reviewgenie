...
export default function Home() {
  ...
  const [platform, setPlatform] = useState<"Google" | "Facebook" | "Neither">(
    "Google",
  );
  ...
  useEffect(() => {
    ...
    if (reviewText.trim().length > 10) {
      ...
      timeoutRef.current = setTimeout(() => {
        detectSentimentMutation.mutate(
          { data: { reviewText } },
          {
            onSuccess: (data) => {
  ...
  const handleGenerate = () => {
    if (!reviewText.trim()) {
      toast({
        title: "Review text is empty",
        description: "Please paste a review first.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleSave = () => {
    ...
    toast({
      ...
      description: "You can view it in the History tab.",
    });
  };
  ...
  const isLoading =
    detectSentimentMutation.isPending || generateReplyMutation.isPending;
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-card/50 backdrop-blur-sm p-8 bg-grid-pattern before:absolute before:inset-0 before:bg-[radial-gradient(ellipse_at_center,rgba(245,158,11,0.15)_0%,transparent_70%)] before:pointer-events-none">
          <div className="relative z-10 flex flex-col items-center text-center space-y-4">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl">
              Paste your customer review below and let AI generate the perfect
                    ...
                    {detectedSentiment && autoDetect && (
                      ...
                      >
                        ...
                        >
                          ...
                          Detected: {detectedSentiment} — Auto-selected{" "}
                          {selectedTone} tone
                        </Badge>
                      </motion.div>
                    )}
                  ...
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        className={cn(
                          "w-6 h-6",
                          starRating >= star ? "fill-amber-500" : "opacity-30",
                        )}
                      />
                    </button>
                  ))}
                  ...
                  <Card className="border-primary/30 shadow-[0_0_30px_rgba(245,158,11,0.1)] overflow-hidden">
                    <CardContent className="p-0">
                      ...
                      <div className="p-6 bg-card">
                        <p className="whitespace-pre-wrap leading-relaxed text-[1.05rem]">
                          {generatedReply}
                        </p>
                      </div>
                    </CardContent>
              ...
              <CardContent className="p-6 space-y-6">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    disabled={autoDetect && !!reviewText.trim()}
                  >
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Used in the signature of the reply
                  </p>
                </div>
